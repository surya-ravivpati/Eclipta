...
function PublicProfilePage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: rows } = await supabase.rpc("get_public_profile", {
        p_username: username,
      });
      ...
      const [{ data: e }, { data: t }, { count: fCount }, followRow] = await Promise.all([
        supabase
          .from("user_ecliptars")
          .select("id,ecliptar_name,ecliptar_slug,archetype")
          .eq("user_id", p.user_id),
        supabase
          .from("forum_threads")
          .select("id,title,created_at,votes,answer_count")
          .eq("user_id", p.user_id)
        ...
        >
          <div
            className={cn(
              ...
              equippedArch
                ? equippedArch.borderColor + " bg-secondary/30"
          ...
          >
            {profile.avatar_url ? (
              ...
              <EquippedIcon
                className={cn("w-12 h-12", equippedArch?.color ?? "text-neon-purple")}
              />
            )}
          </div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div className="glass-panel p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="font-display font-bold text-sm tracking-tight uppercase">
                Ecliptars Owned
              </h2>
            </div>
            {ecliptars.length === 0 ? (
              ...
              <div className="grid grid-cols-3 gap-2">
                {ecliptars.slice(0, 9).map((e) => {
                  ...
                  return (
                    <div
                      key={e.id}
                      className={cn("glass-panel p-3 border text-center", arch?.borderColor)}
                    >
                      <Icon className={cn("w-6 h-6 mx-auto", arch?.color)} />
                      <p className={cn("text-[10px] mt-1 font-bold tracking-widest", arch?.color)}>
                        {e.ecliptar_name}
                      </p>
                    </div>
                  );
