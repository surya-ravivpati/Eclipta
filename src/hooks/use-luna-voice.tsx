...
export function useLunaVoice(opts: { onTranscript: (text: string) => void }) {
  ...
  const transcribe = useCallback(async (blob: Blob, ext: string) => {
    try {
      ...
      if (!r.ok) {
        if (r.status === 404) {
          setVoiceError(
            "Voice input isn't set up on the server yet (the luna-stt function needs to be deployed).",
          );
          return;
        }
      ...
      try {
        ...
        if (!r.ok || ctrl.signal.aborted) {
          if (r.status === 404 && !ctrl.signal.aborted) {
            setVoiceError(
              "Read-aloud isn't set up on the server yet (the luna-tts function needs to be deployed).",
            );
          }
          return;
        }
  ...
  const setSpeakEnabledSafe: typeof setSpeakEnabled = useCallback(
    (v) => {
