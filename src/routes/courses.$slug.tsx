        ...
        <div className="max-w-5xl mx-auto">
          ...
          >
            ...
            <p className="text-sm text-muted-foreground mb-3 inline-flex items-center gap-1.5">
              <UserIcon className="w-3.5 h-3.5" /> by {creatorName}
            </p>
            ...
            {course.summary && (
              <p className="text-base text-foreground/80 leading-relaxed mb-6">{course.summary}</p>
            )}
            <button
              onClick={enroll}
              disabled={enrolled}
          ...
          <div className="grid md:grid-cols-[240px_1fr] gap-6">
            <div className="glass-panel p-4 h-fit md:sticky md:top-24">
              <h3 className="font-display font-bold text-xs tracking-widest text-muted-foreground mb-3">
                SYLLABUS
              </h3>
              ...
              {activeBlocks.map((b) => {
                if (b.type === "text")
                  return (
                    <div
                      key={b.id}
                      className="glass-panel p-5 prose prose-invert max-w-none text-sm whitespace-pre-wrap"
                    >
                      {b.data.text}
                    </div>
                  );
                      ...
                      <div className="space-y-2">
                        {(b.data.options || []).map((opt: string, i: number) => {
                          ...
                          return (
                            <button
                              key={i}
                              onClick={() => setQuizState((s) => ({ ...s, [b.id]: i }))}
                              className={`w-full text-left px-3 py-2 text-sm border transition-colors ${
                                !showResult
                                  ? "border-border hover:border-neon-purple/60 bg-secondary/30"
                                  : isCorrect
