...
export function UserSearchDialog({
  ...
  const challenge = async (target: Result) => {
    if (!user) {
      toast.error("Sign in to challenge.");
      return;
    }
  ...
  const archetypes = useMemo(
    () => ARCHETYPE_IDS.map((id) => ({ id, name: ARCHETYPES[id].name })),
    [],
  );
        ...
        <div className="space-y-3">
          ...
          <div className="max-h-72 overflow-y-auto -mx-2 px-2">
            ...
            {results.map((r) => (
              <div
                key={r.user_id}
                className="flex items-center gap-3 px-2 py-2 border-b border-border/40 last:border-0"
              >
