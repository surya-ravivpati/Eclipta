...
export const ARCHETYPES: Record<ArchetypeId, Archetype> = {
  speedster: {
    ...
    description: "Fast, pressured gameplay. The shortest clock in the roster — but the faster you answer, the harder you hit.",
  ...
  tank: {
    ...
    description: "Heavy, durable, easy questions. The biggest HP pool and the best armour — but the lowest damage, and no way to heal.",
  ...
  chud: {
    ...
    description: "Glass cannon. The hardest hits and the meanest crits on the thinnest HP bar — and it gets deadlier as it dies.",
  ...
  healer: {
    ...
    description: "Sustain-focused: the strongest HP restore, the longest clock, and easy questions. Wins by outlasting, not out-hitting.",
  ...
  fulcrum: {
    ...
    description: "Balanced all-rounder that borrows from the rest of the roster. Never the best at anything — never without an answer.",
  ...
  accelerator: {
    ...
    description: "Scaling power over time. Every correct answer permanently sharpens your damage (14→30) and your score bonus (up to +35%).",
  ...
  god: {
    ...
    description: "Endgame archetype. Towering stats and the hardest questions in the game — sustained by sheer consistency rather than by defence.",
...
export function rollGamblerStats(): GamblerRoll {
  ...
  return {
    ...
    defense: parseFloat((Math.random() * 0.20).toFixed(2)),        // 0–20%
    healAmount: Math.round(Math.random() * 30),                    // 0–30 (independent)
    timeSeconds: 20 + Math.floor(Math.random() * 61),              // 20–80s
    critBonus: parseFloat((Math.random() * 0.40).toFixed(2)),      // 0–40%
    diffMin,
    diffMax,
  };
}
