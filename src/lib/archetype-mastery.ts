...
const THRESHOLDS = [
  ...
  { battles: 4, winRate: 0.3, streak: 3, perfect: 0 }, // Rank II
...
const RANK_FLAVORS: Record<ArchetypeId, readonly string[]> = {
  ...
  tank: [
    ...
    "You are the last line. You always hold.",
    "What moves cannot be moved.",
  ],
  chud: [
    "",
    "The first spark.",
    "Ignition.",
    "Pure devastation, barely controlled.",
    "They don't survive first contact.",
    "Beyond damage. Beyond reason.",
  ],
  healer: [
    "",
    "Still learning patience.",
    "Survival is its own weapon.",
    "You outlast everything.",
    "No one can finish you.",
    "Death cannot find you here.",
  ],
  fulcrum: [
    "",
    "Balance is a myth you're learning.",
    "You begin to understand it.",
    "Every battle is a calculation.",
    "Nothing escapes your model.",
    "Perfect. Inevitable. Balanced.",
  ],
  accelerator: [
    "",
    "The first ember.",
    ...
    "Everything you've answered is ammunition.",
    "Late-game god. They should have ended it earlier.",
  ],
  gambler: [
    "",
    "You rolled the dice.",
    "The house didn't know you were coming.",
    "Risk is just another word for edge.",
    "The probabilities bend for you.",
...
export async function recordBattleMastery(
  ...
  await recordArchetypeMasteryRpc(archetype, won, bestStreak, correct, total, perfect);
}
...
export async function fetchMastery(archetype: ArchetypeId): Promise<ArchetypeMastery | null> {
  ...
  if (!user) return null;
  return getArchetypeMastery(user.id, archetype);
}
...
export async function fetchAllMastery(): Promise<ArchetypeMastery[]> {
  ...
  if (!user) return [];
  return getAllArchetypeMastery(user.id);
}
