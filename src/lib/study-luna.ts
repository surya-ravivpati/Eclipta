...
export async function fetchStuckRequests(roomId: string): Promise<StuckRequest[]> {
  ...
  if (error) {
    console.error("fetchStuckRequests", error);
    return [];
  }
  return data ?? [];
}
...
export async function createStuckRequest(roomId: string, note: string): Promise<string | null> {
  const { error } = await supabase.rpc(
    "create_stuck_request" as never,
    { p_room: roomId, p_note: note } as never,
  );
...
export async function resolveStuckHuman(stuckId: string): Promise<boolean> {
  const { data, error } = await supabase.rpc(
    ...
    { p_stuck: stuckId } as never,
  );
  if (error) {
    console.error("resolveStuckHuman", error);
    return false;
  }
  return !!data;
}
...
export async function triggerStuckAi(stuckId: string): Promise<void> {
  try {
    await callRoom({ mode: "stuck", stuck_id: stuckId });
  } catch (e) {
    console.error("triggerStuckAi", e);
  }
}
 ...
 */
export function gatherRecapEvents(
  stuck: StuckRequest[],
