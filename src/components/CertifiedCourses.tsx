...
import { enrollInCourse, getEnrolledCourseSlugs } from "@/repositories/courses";

const PILLARS = [
  {
    icon: ShieldCheck,
    title: "Vetted Creators",
    description: "Every certified creator is hand-picked and verified by Eclipta",
  },
  {
    icon: Award,
    title: "Clear Outcomes",
    description: "Each course maps to recognized levels of understanding",
  },
  {
    icon: TrendingUp,
    title: "Structured Paths",
    description: "Carefully sequenced content with adaptive checkpoints",
  },
];
...
export function CertifiedCourses() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const slugs = await getEnrolledCourseSlugs(user.id);
      if (cancelled) return;
      setEnrolled(new Set(slugs));
    })();
    return () => {
      cancelled = true;
    };
  ...
  const handleEnroll = async (slug: string, title: string) => {
    ...
    try {
      await enrollInCourse({ user_id: user.id, course_slug: slug, course_title: title });
    } catch (e) {
      toast.error("Could not enroll", { description: e instanceof Error ? e.message : undefined });
      return;
    }
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <p className="text-muted-foreground max-w-lg mx-auto">
            Premium courses built by Eclipta's team and a select group of trusted creators. Higher
            quality, deeper structure, clear outcomes.
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {CERTIFIED_COURSES.map((course, i) => {
            ...
            return (
              ...
              >
                <div
                  className={`inline-flex self-start items-center gap-1.5 px-2 py-0.5 text-[10px] font-bold tracking-widest mb-3 ${
                ...
                >
                  {isEnrolled ? (
                    <>
                      <Check className="w-3 h-3" /> ENROLLED — VIEW
                    </>
                  ) : (
                    "ENROLL"
                  )}
