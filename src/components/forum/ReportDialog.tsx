...
export function ReportDialog({
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const err = await submitReport({
      ...
      targetId,
      category: reason,
      note: details.trim().slice(0, 500) || null,
    });
    setSubmitting(false);
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <div className="flex justify-end gap-2">
            ...
            <button
              type="submit"
              disabled={submitting}
              className="px-5 py-2 text-xs font-bold tracking-widest bg-neon-pink text-foreground hover:opacity-90 disabled:opacity-50 transition-opacity inline-flex items-center gap-2 active:scale-[0.97]"
            >
