...
export function GoalPin({
  ...
  useEffect(() => {
    if (!editingGoal) setGoalDraft(room.goal_text ?? "");
  }, [room.goal_text, editingGoal]);
  ...
  const commitGoal = async () => {
    ...
    setSavingGoal(false);
    if (err) {
      toast.error("Couldn't save the goal", { description: err });
      setGoalDraft(room.goal_text ?? "");
    }
  };
  ...
  const addLink = async () => {
    ...
    if (!url) {
      setLinkErr("That doesn't look like a link (e.g. khanacademy.org/...).");
      return;
    }
    if (links.length >= MAX_LINKS) {
      setLinkErr("Max 3 links - remove one to add another.");
      return;
    }
    setLinkErr(null);
    setSavingLink(true);
