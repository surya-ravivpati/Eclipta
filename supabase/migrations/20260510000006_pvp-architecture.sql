...
-- ════════════════════════════════════════════════════════════════════════
-- PvP-First Competitive Architecture
-- Tables: battle_sessions, pvp_queue, pvp_battles, player_ratings
-- RPCs:   find_pvp_match, update_pvp_rating, get_ghost_session, get_pvp_leaderboard
-- ════════════════════════════════════════════════════════════════════════

-- ── battle_sessions ──────────────────────────────────────────────────────
-- Stores completed battle replays. Used as source data for Ghost PvP —
-- real player sessions matched to opponents when no live player is found.
...
-- ── pvp_queue ────────────────────────────────────────────────────────────
-- Live matchmaking queue. Players are inserted on search start, removed
-- when matched or when they cancel / time out.
...
-- ── pvp_battles ──────────────────────────────────────────────────────────
-- Tracks active and completed live PvP battles. Both players subscribe to
-- a Realtime channel named pvp-battle:{id} to exchange HP events.
...
-- ── player_ratings ───────────────────────────────────────────────────────
-- ELO-style competitive ratings. Updated after every live or ghost battle.
-- Bots never affect rating.
...
-- ── find_pvp_match ───────────────────────────────────────────────────────
-- Atomically matches the calling user with the oldest queued player within
-- ±200 rating points. Creates a pvp_battles row and removes both from queue.
-- Returns JSON: { matched: bool, battle_id?, opponent_username?,
--                 opponent_archetype?, opponent_rating? }
...
-- ── update_pvp_rating ────────────────────────────────────────────────────
-- Standard ELO update. K=32 for first 20 games, K=16 thereafter.
-- Rating floor is 100. Returns the new rating.
...
-- ── get_ghost_session ────────────────────────────────────────────────────
-- Returns a random real player battle session within ±150 rating points.
-- Falls back to any session if no nearby match exists.
-- Only returns sessions with ≥ 3 questions from different users.
...
BEGIN
  ...
  -- Try nearby rating within last 30 days first
  ...
  -- Fallback: any session from any time
  IF NOT FOUND THEN
    SELECT * INTO v_session
    FROM public.battle_sessions
    WHERE user_id         != v_uid
...
-- ── get_pvp_leaderboard ──────────────────────────────────────────────────
-- Rating-based leaderboard (not XP). Only includes players with ≥1 battle.
